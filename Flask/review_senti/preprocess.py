...
def parse_review1(reviews_text1):
    filtered_list = []
    if isinstance(reviews_text1, dict):
        for key, value in reviews_text1.items():
        ...
        if reviews_text1[5] == '"':
            ...
            pattern = re.compile(r'\d+: \[\'(.*?)\'\]', re.DOTALL)
        matches = pattern.findall(reviews_text1)
...
def parse_review4(reviews_text):
    reviews_text1 = ast.literal_eval(reviews_text)
    filtered_list = parse_review1(reviews_text1)
    reviews = []
